...
JETBRAINS_SERVER_ROOT_CA_MOD = (
    "6578234164159649328451564351071453577142054187479153567343970552"
    "4296493322175240416661493525510524910165391672168160366275430763"
    "2457128805443068644284748929421507178622933348669378235847742899"
    "7739207659187701054828515344672962841052505125731517052202243795"
    "0938383502232328217617969014631197945155654111812934428212737553"
    "4752754929526193258755897888515754328964698687671896985405008425"
    "3327660823192609880236543598450727761894514487867582454142533861"
    "4624672735445404713542841662485242256736009734667164809306442927"
    "9847823280592246506886401865455491924420415707803245018138595108"
    "0293599988295202365226207499187299382061347393312469020318076012"
    "6641925808041190320159571015546790129114214244322033353975039792"
    "3736434950558371997170985472272249812750161962720036837656464089"
    "1262167495851480308830294382851849300242865178657105399475897645"
    "6420484242669832240403344153166282923887520542076165237533732729"
    "7032255730262635511533223504109798364923988127319356119135982652"
    "7888344050381137463943019575511677403457881757662907946046667814"
    "0232982492411819174631972803387804539609367469671302281402769016"
    "5596773992550010915823534552020920573926469894452526033833249442"
    "2309297424866022984772706721011974447292226598910383503036995012"
    "80729658274069437"
)
...
def extract_key_str(key_bytes: bytes) -> str:
    """
    从给定的公/私钥字节序列中提取公/私钥字符串。

    该函数尝试将字节序列解码为 UTF-8 字符串，并使用正则表达式提取 PEM 格式的公/私钥字符串，
    除去了"-----BEGIN CERTIFICATE-----"和"-----END CERTIFICATE-----"标记及其中的换行符。

    Params:
        key_bytes (bytes): 包含 PEM 格式公/私钥的字节序列。

    Returns:
        str: 提取并处理后的公/私钥字符串，不包含 PEM 格式的头部和尾部标记以及换行符。

    Exceptions:
        ValueError: 如果输入的字节序列无法解码为有效的 UTF-8 字符串或者未在字符串中找到有效的公/私钥。
    """

    # 支持多种常见的公/私钥格式
    key_patterns = [
        # 定义正则表达式以匹配 PEM 格式的公/私钥字符串，包括起始和结束标记。使用 re.DOTALL 以支持跨行匹配
    ...
    try:
        # 尝试将公/私钥的字节序列解码为 UTF-8 字符串
        key_str = key_bytes.decode("utf-8").strip()
    except UnicodeDecodeError:
        # 如果解码失败，抛出 ValueError 异常，指明不是有效的公钥
        raise ValueError("输入数据不是有效的 UTF-8 编码")

    public_key_str_match = False
    # 尝试所有支持的公/私钥格式
    for pattern in key_patterns:
        # 使用正则表达式在解码后的字符串中搜索公/私钥
        match = re.search(pattern, key_str, re.DOTALL)
        if match:
            public_key_str_match = True
            break

    # 如果没有匹配到公/私钥，抛出 ValueError 异常
    if not public_key_str_match:
        raise ValueError("未找到公/私钥")

    # 从匹配对象中提取公/私钥字符串（不包括 PEM 格式的头尾标记）
...
def extract_public_key_modulus(public_key_str: bytes) -> int:
    ...
    try:
        ...
        public_key_str = extract_key_str(public_key_str)
...
def gen_sign(public_key_bytes: bytes) -> tuple[int, int]:
    ...
    # 使用之前定义的函数提取公钥字符串，移除了 PEM 格式的头尾标记和换行符
    public_key_str = extract_key_str(public_key_bytes)
    # 将 Base64 编码的公钥字符串解码为 DER 格式的字节序列，然后加载为证书对象
    cert = x509.load_der_x509_certificate(base64.b64decode(public_key_str))
    ...
    #     signed=False,
    # )
    filled_sign = pow(sign, public_key.public_numbers().e, public_key.public_numbers().n)
...
def gen_license(
    ...
    if license_id is None:
        license_id = license_dict["licenseId"]
    else:
        license_dict["licenseId"] = license_id
    if licensee_name is not None:
        license_dict["licenseeName"] = licensee_name
    else:
        license_dict["licenseeName"] = ""
    if assignee_name is not None:
        license_dict["assigneeName"] = assignee_name
    else:
        license_dict["assigneeName"] = ""
    if assignee_email is not None:
        license_dict["assigneeEmail"] = assignee_email
    else:
        license_dict["assigneeEmail"] = ""
    license_info = json.dumps(license_dict, separators=(",", ":"))

    # 使用之前定义的函数提取公钥字符串，移除了 PEM 格式的头尾标记和换行符
    public_key_str = extract_key_str(public_key_bytes)
    # 将 Base64 编码的公钥字符串解码为DER格式的字节序列，然后加载为证书对象
    cert = x509.load_der_x509_certificate(base64.b64decode(public_key_str))
    # cert = CryptoPlus.loads(public_key_str).key
    # 从证书中提取公钥对象
    public_key = cert.public_key()
    # 将许可证数据编码为 UTF-8 格式的字节序列
    license_info_encoded = license_info.encode("utf-8")
    # 对许可证数据的字节序列计算 SHA1 摘要
    digest = SHA1.new(license_info_encoded)

    # 从字节序列导入私钥对象
    private_key = RSA.import_key(private_key_bytes)
    # 使用私钥对摘要进行签名
    # rsa_key = CryptoPlus.loads(private_key_bytes)
    # signature = rsa_key.sign(license_info_encoded, "SHA1")
    signature = pkcs1_15.new(private_key).sign(digest)

    # 将签名结果进行 Base64 编码
    sig_results = base64.b64encode(signature)
    # 将许可证数据进行 Base64 编码
    license_info_base64 = base64.b64encode(license_info_encoded)

    # 使用证书的公钥进行验证
    try:
        public_key.verify(
            base64.b64decode(sig_results),
            base64.b64decode(license_info_base64),
    ...
    license_result = (
        f"{license_id}"
        "-"
        f'{license_info_base64.decode("utf-8")}'
        "-"
...
if __name__ == "__main__":
    ...
    # 生成 license 证书
    # with (
    #     open(cert_path / "ca.crt", "wb") as f_cert,
    #     open(cert_path / "ca.key", "wb") as f_key,
    # ):
    ...
    # with (
    #     open(cert_path / "ca.crt", "rb") as f_cert,
    #     open(cert_path / "ca.key", "rb") as f_key,
    #     open(cert_path / "licenses.json", "r") as f_licenses,
    # ):
    #     f_cert_bytes = f_cert.read()
    #     f_key_bytes = f_key.read()
    #     # 生成 power config
    #     new_sign = gen_sign(f_cert_bytes)
    #     new_power_cfg = gen_power_cfg(new_sign[0], new_sign[1])
    #     print(new_power_cfg)
    #
    #     # 生成 license
    #     new_license = gen_license(f_cert_bytes, f_key_bytes, f_licenses.read())
    #     print(new_license)

    # 生成 license server 证书
    # with (
    #     open("ls_ca.crt", "wb") as f_ls_cert,
    #     open("ls_ca.key", "wb") as f_ls_key,
    # ):
    #     new_private_bytes, new_public_bytes = gen_certificate(subject_name="anonymous.lsrv.jetbrains.com", issuer_name="License Servers CA")
    #     f_ls_key.write(new_private_bytes)
    #     f_ls_cert.write(new_public_bytes)

    # 生成 license server power config
    # with (
    #     open("ls_ca.crt", "rb") as f_ls_cert,
    #     open("ls_ca.key", "rb") as f_ls_key,
    # ):
    #     new_sign = gen_sign(f_ls_cert.read())
    #     new_power_cfg = gen_power_cfg(new_sign[0], new_sign[1])
    #     print(new_power_cfg)
